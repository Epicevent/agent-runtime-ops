...
After=agent-runtime-root-action-broker-standalone.service
...
[Unit]
Description=Submit groupware runtime observations every UTC quarter-hour
After=$(basename "$BOOT_RESTORE_UNIT_FILE")
...
install_ops_sudoers() {
  ...
  tmp="$(mktemp)"
  {
    ...
    printf '%s ALL=(root) NOPASSWD: %s nas workspace-write-probe *\n' "$OPS_USER" "$BIN_LINK"
...
check_install() {
  ...
  grep -Fxq 'After=agent-runtime-root-action-broker-standalone.service' "$GROUPWARE_REOBSERVATION_SERVICE_FILE" \
    || die "groupware reobservation service broker ordering is stale"
  grep -Fxq "After=$(basename "$BOOT_RESTORE_UNIT_FILE")" "$GROUPWARE_REOBSERVATION_TIMER_FILE" \
    || die "groupware reobservation timer NAS restore ordering is stale"
