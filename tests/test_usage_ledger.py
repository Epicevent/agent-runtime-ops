...
class UsageReceiptContractTest(unittest.TestCase):
    ...
    def test_install_contract_enables_three_minute_collector_and_scoped_sudoers(
        ...
        self.assertIn("usage cost-estimate --pricing-file", install_text)
        self.assertIn("NOPASSWD: %s usage cost-estimate *", install_text)
        self.assertIn("usage fx-refresh --output", install_text)
        self.assertIn("OnCalendar=*-*-* 01:30:00 Asia/Seoul", install_text)
        self.assertIn('ConditionPathExists=$USAGE_PRICING_FILE', install_text)
        self.assertIn('ConditionPathExists=$USAGE_FX_FILE', install_text)
